...
class DietRequestsController extends Controller
{
    ...
    public function getHome($status){
        ...
        switch ($status) {
            ...
            case '2':
                if(Auth::user()->role == 5){
                    $diet_requests = DietRequest::where('status', '2')->where('idapplicant', Auth::user()->id)->orderBy('id', 'Asc')->get();
                }else{
                    $diet_requests = DietRequest::where('status', '2')->orderBy('id', 'Asc')->get();
                }
            break;
    ...
    public function getDietRequestPdf($id){
        ...
        $subtotales_otras = DB::table('diet_request_details')
                 ...
                 ->where('iddiet_request', $iddiet_request)
                 ->whereIn('iddiet', ['19','20','21','22','23','24','25','26','27'])
                 ->get();
